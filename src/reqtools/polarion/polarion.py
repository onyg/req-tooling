        ...
        try:
            ...
            if value.endswith("Z"):
                try:
                    ...
                    raise PolarionExportDateError(
                        f"Unsupported string format for date: {value}"
                    ) from exc
            else:
                try:
...
class PolarionExporter:
    ...
    ALLOWED_CONFORMANCE = ["SHALL", "SHALL NOT", "MAY", "SHOULD", "SHOULD NOT"]
    ...
    def get_default_test_proc(self, actor):
        ...
        if str(actor) in DEFAULT_TEST_PROCEDURE_MAPPING.keys():
            procedure = TESTPROC_MAPPING.get(
                DEFAULT_TEST_PROCEDURE_MAPPING.get(str(actor)), None
            )
    ...
    def get_test_procedure(self, key, requirement):
        ...
        if procedure is None:
            raise PolarionExportMappingError(
                f"❌ No test procedure mapping found for '{key}'. Source: {requirement.source}; requirement key: {requirement.key}."
            )
        return procedure
    ...
    def map_product_types(self, requirement):
        ...
        try:
            for actor, test_procedure in requirement.test_procedures.items():
                ...
                if product is None:
                    _errors.append(
                        f"❌ No product type mapping found for actor '{actor}'. Source: {requirement.source}; requirement key: {requirement.key}."
                    )
                product_type = {}
                ...
                for tp in test_procedure:
                    try:
                        procedure = self.get_test_procedure(
                            key=tp, requirement=requirement
                        )
            ...
            _errors.print_error(
                f"AttributeError: {e}; Source: {requirement.source}; requirement key: {requirement.key}."
            )
        if _errors:
            raise PolarionExportMappingError("\n".join(_errors))
    ...
    def export(self, output):
        ...
        document_info["date"] = (
            f"{convert_polarion_date_export(value=self.ig_config.date)}"
        )
        ...
        for req in release.requirements:
            ...
            if req.conformance not in self.ALLOWED_CONFORMANCE:
                _errors.append(
                    str(
                        PolarionExportConformanceError(
                            f"Conformance {req.conformance} not allowed; {req.source}; requirement key: {req.key}."
                        )
                    )
                )
                continue
            ...
            req_export["link"] = utils.convert_to_ig_requirement_link(
                base=self.ig_config.link,
                source=req.source,
                key=req.key,
                version=req.version,
            )
        ...
        data["requirements"] = sorted(requirements, key=lambda x: x["key"])

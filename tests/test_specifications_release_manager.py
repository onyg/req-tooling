...
def test_load_previous_with_multiple_releases(manager):
    manager.config.releases = ["1.0.0", "1.1.0", "1.2.0"]
    manager.config.current = "1.2.0"
    
    with patch.object(manager, '_load_requirements', return_value=[Requirement(key="REQ-OLD")]):
        previous = manager.load_previous()
        assert isinstance(previous, Release)
        assert previous.version == "1.1.0"
        assert len(previous.requirements) == 1


def test_load_previous_with_two_releases(manager):
    manager.config.releases = ["1.0.0", "1.1.0"]
    manager.config.current = "1.1.0"
...
def test_load_previous_with_one_release(manager):
    manager.config.releases = ["1.0.0"]
    manager.config.current = "1.0.0"
    
    previous = manager.load_previous()
    assert previous is None


def test_load_previous_with_empty_releases(manager):
    manager.config.releases = []
    manager.config.current = None
